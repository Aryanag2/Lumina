...
#             #         exc_`type`, exc_value, exc_traceback = sys.exc_info()
        ...
        with gr.Blocks() as demo:
            ...
            with assistant_interface:
                with gr.Row():
                    with gr.Column(scale=3):
                        initial_prompt = get_initial_prompt()
                        chatbot = gr.Chatbot(value=[("Hello!", initial_prompt)], height=6000, min_width=3000, bubble_full_width=False, type="messages")
                        with gr.Row():
                            msg = gr.Textbox(placeholder="Insert question here...", scale=4)
                            image_input = gr.Image(type="pil", label="Upload Image (optional)", scale=1)
